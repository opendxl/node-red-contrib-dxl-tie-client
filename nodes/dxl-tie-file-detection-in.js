/**
 * @module DxlTieFileDetectionIn
 * @description Implementation of the
 * `dxl-tie-file-detection in` node
 * @private
 */
...
module.exports = function (RED) {
   ...
   *   the file detection event payload, set as `msg.payload`. If payloadType
   *   is 'bin', `msg.payload` is a raw binary Buffer. If payloadType is 'txt',
   *   `msg.payload` is a String (decoded as UTF-8). If payloadType is 'obj',
   *   is an Object (decoded as a JSON document from the original payload). If
   *   an error occurs when attempting to convert the binary Buffer of the
   *   payload into the desired data type, the current flow is halted with an
   *   error.
   * @private
   * @constructor
   */
  function TieFileDetectionInNode (nodeConfig) {
    ...
    /**
     * Controls the data type for the file detection event payload.
     * @type {String}
     * @private
     */
